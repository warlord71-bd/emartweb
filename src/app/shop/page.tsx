...
export default async function ShopPage({ searchParams }: P) {
  ...
  const [{products,total,totalPages},categories]=await Promise.all([
    getProducts({page,per_page:20,category:sp.category,orderby:(sp.orderby as any)||'date',order:(sp.order as any)||'desc',on_sale:sp.on_sale==='true',min_price:sp.min_price,max_price:sp.max_price}),
    getCategories(),
  ]);
  return (
      ...
      <div className="flex gap-6">
        ...
        <aside className="hidden lg:block w-52 flex-shrink-0 space-y-5">
          <div>
            ...
            <div className="space-y-1">
              {[{name:'All',id:''},...categories.slice(0,14)].map(c=>(
                <a key={c.id||'all'} href={`/shop?${c.id?`category=${c.id}`:''}${sp.orderby?`&orderby=${sp.orderby}`:''}`}
          ...
          <div>
            ...
            <a href={`/shop?on_sale=${sp.on_sale==='true'?'':'true'}`}
